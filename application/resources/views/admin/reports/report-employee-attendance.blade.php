@extends('layouts.default')
                                ...
                                <select name="employee" class="ui search dropdown getid">
                                    ...
                                    @isset($employee)
                                        @foreach($employee as $e)
                                            <option value=" {{ $e->firstname }}, {{ $e->lastname }}," data-id="{{ $e->idno }}">{{ $e->firstname }} {{ $e->lastname }}</option>
                                        @endforeach
                                    @endisset
                                </select>
                            </div>
    ...
    $('#dataTables-example').DataTable({responsive: true,pageLength: 30,lengthChange: false,searching: false,ordering: true});
...
"<td>" + employee[i].idno + "</td>" +
                    ...
                    $('#dataTables-example').DataTable({responsive: true,pageLength: 30,lengthChange: false,searching: false,ordering: false});
                }            
            }
        })
    });
