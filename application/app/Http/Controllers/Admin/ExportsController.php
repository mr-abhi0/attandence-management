...
class ExportsController extends Controller
{
	...
	function employeeList()
	{
		...
		foreach ($p as $d) {
			Storage::prepend($file, $d->id . ',' . $d->firstname . ' ' . $d->lastname . ' ' . $d->mi . ',' . $d->idno . ',' . $d->age . ',' . $d->gender . ',' . $d->civilstatus . ',' . $d->mobileno . ',' . $d->emailaddress . ',' . $d->employmenttype . ',' . $d->employmentstatus);
		}

		Storage::prepend($file, '"ID"' . ',' . 'EMPLOYEE' . ',' . 'DP Code' . ',' . 'AGE' . ',' . 'GENDER' . ',' . 'CIVILSTATUS' . ',' . 'MOBILE NUMBER' . ',' . 'EMAIL ADDRESS' . ',' . 'EMPLOYMENT TYPE' . ',' . 'EMPLOYMENT STATUS');
	...
	function attendanceReport(Request $request) 
	{
		...
		if ($id == null AND $datefrom == null AND $dateto == null) 
		{
			$data = table::attendance()
				->join('tbl_company_data', 'tbl_people_attendance.reference', '=', 'tbl_company_data.reference')
				->get();
			$date = date('Y-m-d');
			$time = date('h-i-sa');
			...
			foreach ($data as $d) 
			{
				Storage::prepend($file, $d->id .','. $d->idno .','. $d->date .','. '"'.$d->employee.'"' .','. $d->company . ',' .  $d->timein .','. $d->timeout .','. $d->totalhours .','. $d->status_timein .','. $d->status_timeout);
			}

			Storage::prepend($file, '"ID"' .','. 'DP Code' .','. 'DATE' .','. 'EMPLOYEE' .','. 'Company' . ',' . 'TIME IN' .','. 'TIME OUT' .','. 'TOTAL HOURS' .','. 'STATUS-IN' .','. 'STATUS-OUT');
	...
	function birthdaysReport() 
	{
		...
		foreach ($c as $d) 
		{
		    Storage::prepend($file, $d->idno .', '.$d->firstname.' '. $d->lastname.' '.$d->mi .','. $d->department .','. $d->jobposition .','. $d->birthday .','. $d->mobileno);
		}
